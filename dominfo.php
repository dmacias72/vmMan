<div class="wrap">
   ...
   <?php
   	...
   	$dom = $lv->domain_get_info($res);
		if ($action) {
			...
			elseif ($action == 'disk-add') {
				...
				if ($img)
					$msg = $lv->domain_disk_add($domName, $_POST['img'], $_POST['dev']) ? 
			...
			elseif ($action == 'nic-remove') {
				if ((array_key_exists('confirm', $_GET)) && ($_GET['confirm'] == 'yes'))
					$msg = $lv->domain_nic_remove($domName, $_GET['mac']) ? 
						'Network card has been removed successfully' : 
						'Cannot remove network card: '.$lv->get_last_error();
			}
			elseif ($action == 'nic-add') {
				$mac = array_key_exists('mac', $_POST) ? $_POST['mac'] : false;
				if ($mac)
					$msg = $lv->domain_nic_add($domName, $_POST['mac'], $_POST['network'], $_POST['model']) ? 
      ...
      if ($lv->domain_is_running($res, $name))
         ...
         $tmp = array_merge($tmp,$lv->get_cdrom_stats($domName));
         if (!empty($tmp)) {
