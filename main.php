...
<?php
   ...
   for ($i = 0; $i < sizeof($doms); $i++) {
      ...
      $cpu = $dom['nrVirtCpu'];
		$auto = $lv->domain_get_autostart($res) ? "checked":"";  
      ...
      if ($lv->domain_is_running($res, $name)){
			echo "<button class=\"btn btn-sm btn-primary\" onClick=\"window.open('$vnc','_blank','scrollbars=yes,resizable=yes'); return false;\" 
            	...
            	<button class=\"btn btn-sm btn-primary\" onClick=\"javascript:location.href='?subaction=domain-restart&amp;uuid=$uuid'\" 
              	...
              	<button class=\"btn btn-sm btn-default\" href=\"?subaction=domain-destroy&amp;uuid=$uuid\" 
      ...
      if ($lv->domain_is_running($res, $name))
			echo " | <button class=\"btn btn-sm btn-info\" onClick=\"javascript:location.href='?action=domain-edit&amp;uuid=$uuid&amp;view=readonly'\" 
      	...
      	echo " | <a class=\"btn btn-sm btn-danger\" href=\"?subaction=domain-undefine&amp;uuid=$uuid\" 
          		 onClick=\"return confirm('Are your sure you want to remove $name?')\" title=\"delete domain definition\"><i class=\"glyphicon glyphicon-remove\"></i></a>
          		  | <button class=\"btn btn-sm btn-info\" onClick=\"javascript:location.href='?action=domain-edit&amp;uuid=$uuid&amp;view='\" 
	...
	if ($refresh) echo '<meta http-equiv="refresh" content="3; url=?vmpage=&amp;refresh2=true">';
	if ($_GET['refresh2']) echo '<meta http-equiv="refresh" content="5; url=?vmpage=&amp;refresh=true">';
	if ($_GET['refresh']) echo '<meta http-equiv="refresh" content="7; url=/VMs">';
}
?>
	</div>
</div>